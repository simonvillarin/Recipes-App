...
function American() {
  ...
  const getDishes = async () => {
    try {
      ...
      if (american) {
        setDishes(JSON.parse(american));
      } else {
        const res = await fetch(
          'https://api.spoonacular.com/recipes/random?apiKey=0386ce9e5c044718b7bea7b05be25dc2&cuisine=American&number=16'
        );
        const data = await res.json();

        localStorage.setItem('american', JSON.stringify(data.recipes));
