...
function Japanese() {
  ...
  const getDishes = async () => {
    try {
      ...
      if (japanese) {
        setDishes(JSON.parse(japanese));
      } else {
        const res = await fetch(
          'https://api.spoonacular.com/recipes/random?apiKey=0386ce9e5c044718b7bea7b05be25dc2&cuisine=Japanese&number=16'
        );
        const data = await res.json();

        localStorage.setItem('japanese', JSON.stringify(data.recipes));
