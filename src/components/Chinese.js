...
function Chinese() {
  ...
  const getDishes = async () => {
    try {
      ...
      if (chinese) {
        setDishes(JSON.parse(chinese));
      } else {
        const res = await fetch(
          'https://api.spoonacular.com/recipes/random?apiKey=0386ce9e5c044718b7bea7b05be25dc2&cuisine=Chinese&number=16'
        );
        const data = await res.json();

        localStorage.setItem('chinese', JSON.stringify(data.recipes));
